...
public class Player_Stats : Entity_Stats
{
    ...
    public void ApplyBuff(BuffEffect_Data[] buffsToAply, float duration, string source)
    {
        StartCoroutine(BuffCo(buffsToAply, duration, source));
    }

    private IEnumerator BuffCo(BuffEffect_Data[] buffsToApply, float duration, string source)
    {
