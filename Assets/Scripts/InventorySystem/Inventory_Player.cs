...
public class Inventory_Player : Inventory_Base
{
    ...
    public void SetQuickItemInSlot(int slotNumber, Inventory_Item itemToSet)
    {
        ...
        {
            if (i != targetIndex && quickItems[i] == itemToSet)
            {
                quickItems[i] = null;
                OnQuickSlotUsed?.Invoke(i, null);
            }
        }

        quickItems[targetIndex] = itemToSet;
        OnQuickSlotUsed?.Invoke(targetIndex, itemToSet);
    }
    ...
    public override void SaveData(ref GameData data)
    {
        ...
        foreach (var item in itemList)
        {
            if (item != null && item.itemData != null)
            {
                ...
                {
                    Debug.LogWarning("Item '" + item.itemData.name + "' has no saveId — skipping save");
                    continue;
                }
